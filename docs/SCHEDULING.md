...
The scheduler calls the existing `SendZDOs(peer, false)` operation. It preserves serialization, relevance discovery, forced sends, destroy processing, queue allowances and native transport settings. Explicit flush paths still run vanilla code. Player hosts, clients, and servers with ready non-Steam peers retain vanilla scheduling.
...
Container cgroup throttling, per-message oldest queue age, client CPU reports, ownership reassignment and native rate/window tuning remain separate future work. No new network protocol or client telemetry messages are introduced here.
