...
The Mono test executables require Harmony's runtime dependencies as well as `0Harmony.dll`: `MonoMod.RuntimeDetour.dll`, `MonoMod.Utils.dll`, and `Mono.Cecil.dll`. Reference preparation extracts these from the same checksum-verified BepInEx archive. The test projects import `tests/HarmonyRuntime.props`, which checks for the files and copies them into fresh test outputs. These dependencies remain private test/build inputs and are not added to the mod ZIP.

On Ubuntu/Debian, the real-game network checks also require `libmono-system-net-http4.0-cil`, installed by the mod workflow alongside Mono's core libraries. Valheim's `ZNet` contains an `HttpClient` field; without this runtime package, loading the type can fail with `ZNet:m_httpClient` / `netstandard, Version=2.1.0.0` even though compilation succeeds. Install Mono's HTTP package rather than copying Unity's framework DLLs into the test output. The mod still targets `net48`. After pushing a workflow fix, start a new run so it uses the updated package list.
