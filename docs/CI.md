...
2. Open [Draft a new release](https://github.com/NekoShinobi/valheim-boosted/releases/new). Choose or create the matching tag, such as `v0.1.0`, and select its target branch. Enter a title and write the description in the large **Describe this release** box. Check **This is a pre-release** for pre-alpha builds. Keep the tag numeric; do not append `-alpha`.
...
4. Use the [draft release flow above](#create-a-github-release-or-pre-release) with the matching `vMajor.Minor.Patch` tag. Preparation checks the tag against the source versions before building. Direct tag pushes remain supported for CI artifact/image builds.
