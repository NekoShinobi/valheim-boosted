...
- A C# BepInEx/Jötunn mod: diagnostics HUD, server/peer metrics, optional client performance RPCs, an experimental fair replication scheduler and opt-in Stage 3–5 improvements.
...
| `mod/AdvancedReplication.cs`, `SendWindowPolicy.cs`, `SteamRate*`, `Captain*`, `ZdoCompression.cs`, `CompressionSession.cs`, `LosslessZdoCodec.cs` | Opt-in Stages 3–5; see `docs/SERVER-IMPROVEMENTS.md` |
...
| `dashboard/client/` | Svelte views and LayerChart SVG charts |
...
| `thunderstore/` | Manifest, package README and icon; root `CHANGELOG.md` is packaged too |
...
| `INSTALLATION-REPORT.md`, `docs/*-REPORT.md` | Owner's local planning/session/installation records; intentionally excluded from publication. |
