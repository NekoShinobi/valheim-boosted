...
<p align="center"><strong>See what your connection is doing.</strong><br>0.3.0 · Pre-alpha · Valheim networking diagnostics</p>
...
Import `valheim-boosted-0.3.0.zip` into a Valheim profile in r2modman or Thunderstore Mod Manager, with **BepInExPack Valheim** and **Jötunn** installed.

For a manual Linux installation, download **`valheim-boosted-0.3.0-plugins.zip`**, stop the server, and extract directly into its plugins directory:

```sh
unzip -o valheim-boosted-0.3.0-plugins.zip -d /path/to/BepInEx/plugins
```
...
To release, write your notes in GitHub's **Draft a new release** editor, choose the pre-release checkbox if needed, and save the draft. Choose an unused numeric tag such as `0.1.0` and target `main`. Run **Prepare GitHub release** with that tag: it commits the version files, changelog notes and README version updates, builds that commit, then creates the tag and attaches both ZIPs and checksums. Publish from the editor when ready. [Release steps →](docs/CI.md#create-a-github-release-or-pre-release)
