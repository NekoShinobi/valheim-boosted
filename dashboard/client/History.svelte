...
<div class="shell">
  <aside class="sidebar"><div class="brand-mark">VB<span>↗</span></div><div><strong>valheim-boosted</strong><p class="muted">World observability</p></div><a href="/">◈ Overview</a><a class="nav-active" href="/history">◷ History</a><a href="/reports">▤ Reports</a><div class="sidebar-footer"><span class="eyebrow">ONE SHARED TIMELINE</span><p>Compare the server, client performance and both sides of a connection. Missing samples remain gaps.</p></div></aside>
  <main>
    <header><div><p class="eyebrow">FIND THE MOMENT</p><h1>{archived ? 'Saved timeline' : 'Player & server history'}</h1><p class="muted">{archived ? reportName : `${status?.retentionDays ?? 7}-day retention · updated every 10 seconds`} · {timezone}</p></div><a class="outline" href="/">← Live overview</a></header>
    ...
    <section class="panel">
      ...
      <div class="history-grouping"><label class="checkbox-label"><input type="checkbox" bind:checked={groupPlayers} onchange={()=>{manualSelection=false;void refresh();}} />Combine returning players across sessions</label><p class="muted">Uses server-scoped Steam account identity. Character names are labels; older data without identity remains separate.</p></div>
      <details class="history-sources"><summary>Choose players and sessions · {selected.length}/9 series</summary>
        {#if truncated}<p class="muted">Showing the most recent 256 series. Narrow the time range to find older sessions.</p>{/if}
        <div class="source-grid">{#each sources as s (s.id)}<label class="checkbox-label"><input type="checkbox" checked={selected.includes(s.id)} disabled={!selected.includes(s.id) && selected.length >= 9} onchange={e => toggle(s.id, e.currentTarget.checked)} /><span>{s.name}<small>{s.grouped?`${s.sessionCount} sessions combined`:s.kind} · last seen {date(s.lastMs)} · {(s.playerId??s.id).slice(-6)}</small></span></label>{:else}<p class="muted">No history in this range. Server samples appear once telemetry is live; player frame metrics require a reporting client.</p>{/each}</div>
      </details>
    </section>
    <div class="history-metric"><label><span id="history-metric-label">Compare</span><select aria-labelledby="history-metric-label" bind:value={metric} onchange={() => void refresh()}>{#each historyMetrics as m}<option value={m.key}>{m.label}</option>{/each}</select></label><p class="muted">Hover charts together, or click a chart to inspect a time. Window percentiles are not whole-session percentiles.</p></div>
    ...
    <section class="panel range-summary"><div class="panel-heading"><div><h2>Across the selected range</h2><p class="muted">Weighted by observed frames or samples. Offline gaps add no measurements.</p></div></div>
      <div class="table-scroll"><table><thead><tr><th>Player / perspective</th><th>{definition.label}</th><th>RTT</th><th>Observed time</th><th>Sessions</th></tr></thead><tbody>
        {#each chosen as s}{@const summary=results[0]?.series.find(r=>r.info.id===s.id)?.summary}<tr><td>{s.name}<small>{s.playerId?`Player ${s.playerId.slice(-6)}`:'Session-specific data'}</small></td><td>{number(summary?.value)} {definition.unit.replace('/bucket','')}</td><td>{number(results[1]?.series.find(r=>r.info.id===s.id)?.summary?.value)} ms</td><td>{number(summary ? summary.observedMs/1000 : null)} s</td><td>{s.sessionCount??(s.sessionId?1:'—')}</td></tr>{/each}
      </tbody></table></div>
    </section>
    <section class="panel">
    ...
    <section class="panel"><div class="panel-heading"><div><h2>Player lag markers</h2><p class="muted">F9 records when the client processes the key. After a freeze, this may be later than the incident.</p></div></div><div class="history-markers">{#each events as e}<button class="outline" onclick={() => cursor = e.at}>{sources.find(s => s.id === e.seriesId)?.name ?? 'Client'} · {date(e.at)} · ±{number(e.clockErrorMs)} ms</button>{:else}<p class="muted">No markers in the selected range and series.</p>{/each}{#if results[0]?.markersTruncated}<p class="muted">Marker limit reached; narrow the time range.</p>{/if}</div></section>
    <p class="muted">Client values are self-reported. Focus, frame limits, loading, GC, and shared workload can explain coincident slowdowns; correlation does not establish which player or process caused lag. Clock uncertainty includes round-trip delay and an allowance for drift. Server frame intervals include waiting and frame limiting.</p>
    <footer><span>valheim-boosted / History</span><span>{archived ? 'Saved independently of live retention' : `Database ${number((status?.databaseBytes ?? 0) / 1048576)} MiB · ${status?.droppedOffers ?? 0} history offers replaced while busy`}</span></footer>
  </main>
</div>
