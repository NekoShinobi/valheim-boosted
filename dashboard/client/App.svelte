<script lang="ts">
  ...
  let revision = 0;
  let section = $state<'overview' | 'connections' | 'server' | 'diagnostics'>('overview');
  const sections = [
    { id: 'overview', label: 'Overview' }, { id: 'connections', label: 'Connections' },
    { id: 'server', label: 'Server' }, { id: 'diagnostics', label: 'Diagnostics' },
  ];
  function readSection() {
    const hash = location.hash.slice(1);
    if (hash === 'main-content') return;
    section = ['runtime', 'replication', 'ownership', 'improvements', 'server'].includes(hash) ? 'server'
      : ['diagnostics', 'connection-diagnostics'].includes(hash) ? 'diagnostics'
      : ['connections', 'client-telemetry'].includes(hash) ? 'connections' : 'overview';
  }
  ...
  onMount(() => {
    ...
    void poll();
    return () => { window.removeEventListener('hashchange', readSection); stopped = true; clearTimeout(timer); controller?.abort(); };
  });
</script>

<svelte:head><title>Live · valheim-boosted</title></svelte:head>
<DashboardShell page="live">
  <header class="page-heading">
    <h1>{section === 'overview' ? 'Live overview' : sections.find(s => s.id === section)?.label}</h1>
  ...
  <nav class="section-nav" aria-label="Live sections">
    {#each sections as item}<a href={`#${item.id}`} aria-current={section === item.id ? 'page' : undefined}>{item.label}</a>{/each}
  </nav>
    {#if status !== 'live'}
      <div class="notice" role="status">{error ? 'Connection lost. Showing the last received measurements.' : data?.message ?? 'Connecting to the metrics service…'}</div>
    {/if}
    {#if blockedProbes > 0}<div class="notice" role="status">{blockedProbes} {blockedProbes === 1 ? 'feature' : 'features'} unavailable. <a href="#diagnostics">View diagnostics.</a></div>{/if}
    {#if unavailablePeers.length > 0}<div class="notice" role="status">Measurements unavailable for {unavailablePeers.length} of {peers.filter(p => p.connected).length} connected peers. <a href="#connection-diagnostics">View diagnostics.</a></div>{/if}
  {#if section === 'overview'}
    <div class="cards">
      <section class="stat"><span class="eyebrow">CONNECTIONS</span><strong>{snapshot ? peers.length : '—'}</strong><small>{snapshot?.role === 'dedicated_server' ? 'Dedicated server' : 'This process'}</small></section>
    ...
    <div class="toolbar"><div class="segmented"><button class:chosen={metric === 'rttMs'} aria-pressed={metric === 'rttMs'} onclick={() => metric = 'rttMs'}>Latency</button><button class:chosen={metric === 'estimatedTransportQueueMs'} aria-pressed={metric === 'estimatedTransportQueueMs'} onclick={() => metric = 'estimatedTransportQueueMs'}>Queue delay</button></div><div class="chart-scope"><span class="muted">{data?.history.length ?? 0} recent samples · {selected ? short(selected) : 'Up to 8 peers'}</span>{#if selected}<button class="outline" onclick={() => selected = ''}>All connections</button>{/if}</div></div>
  ...
  {:else if section === 'connections'}
    <section class="panel" id="connections">
      <div class="panel-heading"><div><h2>Connections</h2><p class="muted">Select a connection to inspect its queues.</p></div>{#if selected}<button class="outline" onclick={() => selected = ''}>Show all</button>{/if}</div>
  ...
  {:else}
    <section class="panel" id="diagnostics">
      <div class="panel-heading"><div><h2>Features & compatibility</h2><p class="muted">Game {snapshot?.compatibility?.gameVersion ?? 'unknown'} · Protocol {snapshot?.compatibility?.networkVersion ?? 'unknown'} · {snapshot?.compatibility?.status ?? 'Compatibility not reported'}</p></div></div>
      <details class="measurement-notes"><summary>Build &amp; interfaces</summary>
      <p class="muted build-id">Mod build: {snapshot?.modBuildId ?? 'not reported by this mod build'}</p>
      <p class="muted">Steam interface: {snapshot?.compatibility?.steamInterface ?? 'Unavailable'}. Restart Valheim to apply feature changes.</p>
      </details>
      ...
      <div class="connection-diagnostics" id="connection-diagnostics">
        ...
        <p class="muted">Failed measurements are retried each sample.</p>
        {#each unavailablePeers as peer (peer.peerSessionId)}
          <ConnectionDiagnostics {peer} />
        {:else}<p class="muted">{peers.length ? 'All connected peers are reporting measurements.' : 'Awaiting a connected peer.'}</p>{/each}
      </div>
    </section>
  {/if}
  <footer><span>Mod {snapshot?.modVersion ?? '—'}</span><span>— Unavailable</span></footer>
</DashboardShell>
